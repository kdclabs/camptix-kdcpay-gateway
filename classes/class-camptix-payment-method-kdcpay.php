...
class CampTix_Payment_Method_KDCpay extends CampTix_Payment_Method
{
	...
	public function payment_checkout($payment_token)
	{
		...
		foreach ($attendees as $attendee) {
			...
			if ($attendee_comms_id != '') { // Check if Setup for Communication Preference is set?
				$attendee_info_comms = $attendee_questions[$attendee_comms_id];
			} else {
				$attendee_info_comms = '';
			}
			...
			$attendee_info[] = array(
				...
				$attendee_info_country,
				$attendee_info_comms
			); // array(0=id,1=email,2=first_name,3=last_name,4=tix_amount,5=tix_name,6=access_token,7=edit_token,8=mobile,9=country,10=comms);
		}
